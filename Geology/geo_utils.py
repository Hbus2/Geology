...
# Resolve the CSV next to THIS file, so it works no matter the working directory
# (important on Streamlit Cloud, where the cwd is the repo root, not the app folder).
DATA_PATH = Path(__file__).resolve().parent / "chattanooga_geology.csv"
...
def load_data(path=DATA_PATH):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Could not find {path.name}. Make sure chattanooga_geology.csv is "
            f"committed to the repo in the SAME folder as the app files. Looked in: {path.parent}"
        )
    df = pd.read_csv(path)
